...
const MD_SOURCE_ROOT = path.join(process.cwd(), 'ordergroove-md');
...
function loadSearchIndex() {
  try {
    // For now, skip MiniSearch loading due to ES module compatibility issues
    // The search functionality will work with a simple text-based search
    console.log('Search index loading skipped - using fallback search');
    MINI = null;
  } catch (e) {
  ...
  switch (name) {
      ...
      if (MINI) {
        ...
        try {
          ...
          const results = SECTIONS
            .filter(section => 
              section.text?.toLowerCase().includes(queryLower) || 
              section.headingPath?.toLowerCase().includes(queryLower)
            )
            .slice(0, limit)
            .map(section => ({
          ...
          return {
            content: [
              {
                type: 'text',
                text: `Found ${results.length} results for "${query}" (fallback search):\n\n` +
                  results.map((r: any) => 
                    `**${r.headingPath}**\n${r.snippet}\n*File: ${r.filePath}*\n`
                  ).join('\n---\n'),
              },
    ...
    case 'get_file': {
      ...
      try {
        ...
        const mdSourceResolved = path.resolve(MD_SOURCE_ROOT);
        if (!resolvedPath.startsWith(dataDirResolved) && !resolvedPath.startsWith(mdSourceResolved)) {
          throw new Error('File path must be within the data directory or Markdown source directory');
        }
...
async function main() {
  ...
  console.error('Ordergroove MCP server running on stdio');
}
