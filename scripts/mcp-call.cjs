...
function run(query, cwd) {
  return new Promise((resolve, reject) => {
    const cp = spawn('npm', ['run', 'dev', '--silent'], { cwd, stdio: ['pipe', 'pipe', 'inherit'] });
...
run(query, cwd).catch(err => { console.error(err.message || String(err)); process.exit(1); });
